...
- If OpenCLIP or Torch is missing, the CLIP wrapper returns deterministic random embeddings. This is for debugging, not meaningful retrieval.
- Object detection is optional and depends on local model availability.
  - If local_files_only is false, the detector may download models.
- Environment prompts are intentionally small and coarse; you can extend them in config.yaml or indexer/models/env_classifier.py.
- Weather is parsed from the query but currently not used in scoring.
- Attribute filters are heuristic by design; they trade recall for precision.
...
- Add richer garment prompts or a fashion-specific segmentation model.
- Replace greedy assignment with a learned cross-encoder reranker.
- Expand environment prompts with weather, indoor/outdoor, and seasonal cues.
- Add a small evaluation script to track retrieval quality over time.
